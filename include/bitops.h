					  ...
					  size_t old_bits, size_t new_bits)
{
	...
	unsigned long *new_bmap =  wrealloc(old_bmap, new_size);
...
{
	watomic_or(addr + nr / BITS_PER_LONG, 1UL << (nr % BITS_PER_LONG));
}

static inline void atomic_clear_bit(int nr, unsigned long *addr)
{
	watomic_and(addr + nr / BITS_PER_LONG, ~(1UL << (nr % BITS_PER_LONG)));
}
