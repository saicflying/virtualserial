...
#define rb_for_each(pos, tmp, root)						\
	for (tmp = (pos = rb_first(root), NULL);	\
	     pos && (tmp = rb_next(pos), 1);			\
	     pos = tmp)

/* Iterate over a rbtree of given type safe against removal of rbnode */
#define rb_for_each_entry(pos, tmpp, tmpn, root, member)	\
	for (tmpp = rb_first(root);	\
	     tmpp && (tmpn = rb_next(tmpp), 1) &&		\
		     (pos = rb_entry(tmpp, typeof(*pos), member), 1); \
	     tmpp = tmpn)
...
({									\
	...
	rb_for_each_entry(__dummy, tmpp, tmpn, root, member) {			\
...
({									\
	type *__src, *__dst;						\
 	struct rb_node *tmpp, *tmpn;					\
	rb_for_each_entry(__src, tmpp, tmpn, root, member) {		\
		__dst = wmalloc(sizeof(*__dst));			\
		*__dst = *__src;					\
