...
void trace_function_enter(unsigned long ip, unsigned long *ret_addr)
{
	...
	soe_assert(ret_stack_index < ARRAY_SIZE(trace_ret_stack));

	caller = trace_lookup_ip(ip);

	list_for_each_entry(tracer, &tracers, list) {
		if (tracer->enter && watomic_is_true(&tracer->enabled)) {
			tracer->stack_depth++;
...
unsigned long trace_function_exit(void)
{
	...
	list_for_each_entry(tracer, &tracers, list) {
		if (tracer->exit && watomic_is_true(&tracer->enabled)) {
...
int trace_enable(const char *name)
{
	...
	if (tracer == NULL) {
		soe_debug("no such tracer, %s", name);
		return SOE_RES_NO_SUPPORT;
	} else if (watomic_is_true(&tracer->enabled)) {
		soe_debug("tracer %s is already enabled", name);
		return SOE_RES_INVALID_PARMS;
	}

	watomic_set_true(&tracer->enabled);
	...
	soe_debug("tracer %s enabled", tracer->name);
...
int trace_disable(const char *name)
{
	...
	if (tracer == NULL) {
		...
		soe_debug("tracer %s is not enabled", name);
		return SOE_RES_INVALID_PARMS;
	}

	watomic_set_false(&tracer->enabled);
	if (count_enabled_tracers() == 0) {
		suspend_worker_threads();
		nop_all_sites();
		resume_worker_threads();
	}
	soe_debug("tracer %s disabled", tracer->name);
...
int trace_buffer_pop(void *buf, uint32_t len)
{
	...
	for (i = 0; i < nr_cpu; i++) {
		soe_mutex_lock(&buffer_lock[i]);
		readin = strbuf_stripout(&buffer[i], buff, len);
		soe_mutex_unlock(&buffer_lock[i]);
		count += readin;
		if (count == requested)
			return count;
		if (readin == 0)
...
{
	soe_mutex_lock(&buffer_lock[cpuid]);
	strbuf_add(&buffer[cpuid], item, sizeof(*item));
	soe_mutex_unlock(&buffer_lock[cpuid]);
}
...
static int init_callers(void)
{
	...
	for (int i = 0; i < symcount; i++) {
		...
		if (ip == 0) {
			soe_debug("%s doesn't have mcount call", name);
			continue;
		}
...
int trace_init(void)
{
	...
	for (i = 0; i < nr_cpu; i++) {
		...
		soe_init_mutex(&buffer_lock[i]);
	}

	soe_info("trace support enabled. cpu count %d.", nr_cpu);
	return 0;
}
