...
char *build_short_options(const struct soe_option *soe_opts)
{
	static char sopts[256], *p;
	const struct soe_option *opt;

	p = sopts;
	soe_for_each_option(opt, soe_opts) {
		*p++ = opt->ch;
		if (opt->has_arg)
			*p++ = ':';
	}
...
struct option *build_long_options(const struct soe_option *soe_opts)
{
...
const char *option_get_help(const struct soe_option *soe_opts, int ch)
{
	const struct soe_option *opt;
